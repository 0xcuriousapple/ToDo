...
export default function PriorityIcon(props) {
    ...
    const handleClick = () => {
        ...
        switch (Pri) {
            case 'high': temp = 'low';
                break;
            case 'medium': temp = 'high';
                break;
            case 'low': temp = 'medium';
                break;
        }
    ...
    const colorSelector = (id) => {
        switch (id) {
            case 'high': return '#FF9892'
            case 'medium': return '#FFE692';
            case 'low': return '#97FF92';

        }
    };
    return (
        <span>
            <IconButton style={({ color: colorSelector(Pri) })} aria-label="date" size="small" onClick={handleClick}>
